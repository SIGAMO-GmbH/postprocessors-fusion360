/**
  ...
  $Revision: 43828 4eee7a95f7596e37eed4841d423fce7d59788790 $
  $Date: 2022-06-08 12:58:06 $

  FORKID {2733B062-3BBF-4ED4-A43A-5D86979C354E}
*/

description = "SIGAMO RoboDrill";
vendor = "marloth automation";
...
properties = {
  ...
  flush: {
    title      : "Bettbahnsp.",
    description: "Activates flushing pump",
    group      : "SIGAMO",
    type       : "boolean",
    value      : true,
    scope      : "post"
  },
  waitForInternalCooling: {
    title      : "Warten bei IK.",
    description: "Fügt ein G4 ein, der anhängig vom Werkzeugdurchmesser wartet, bis die Leitung entlüftet ist.",
    group      : "SIGAMO",
    type       : "boolean",
    value      : true,
    scope      : "post"
  ...
  rapidToolChange: {
    title      : "Werkzeugwechsel in sicherer Position",
    description: "Tisch verfährt in Sicherheitsposition vor Werkzeugwechsel.",
    group      : "SIGAMO",
    type       : "boolean",
    value      : true,
    scope      : "post"
...
var coolants = [
  ...
  {id:COOLANT_MIST},
  {id:COOLANT_THROUGH_TOOL, on:22, off:24},
  {id:COOLANT_AIR},
  {id:COOLANT_AIR_THROUGH_TOOL,on:23, off:24},
  {id:COOLANT_SUCTION},
  {id:COOLANT_FLOOD_MIST},
  {id:COOLANT_FLOOD_THROUGH_TOOL, on:[8, 22], off:[9]},
...
var bOutput = createVariable({prefix:"XXXXX"}, abcFormat);
var cOutput = createVariable({prefix:"B"}, abcFormat);
...
function onOpen() {
  ...
  if (getProperty("fifthAxis")) { // note: setup your machine here
    var aAxis = createAxis({coordinate:0, table:true, axis:[1, 0, 0], range:[-110, 90]});
  ...
  if (getProperty("writeTools")) {
    ...
    if (tools.getNumberOfTools() > 0) {
      for (var i = 0; i < tools.getNumberOfTools(); ++i) {
        ...
        var comment = "T: [" + toolFormat.format(tool.number) + "] - Wzg.Nr [" + tool.comment + "] - Offset: [" + tool.lengthOffset + "] - " +
          "D=" + xyzFormat.format(tool.diameter)// + " " +
          //"R=" + xyzFormat.format(tool.diameter)/2 + " " //+
          //localize("Eckradius") + "=" + xyzFormat.format(tool.cornerRadius);
        if ((tool.taperAngle > 0) && (tool.taperAngle < Math.PI)) {
          comment += " " + localize("Winkel") + "=" + taperFormat.format(tool.taperAngle) + localize("deg");
        }
        if (zRanges[tool.number]) {
  ...
  if (useMultiAxisFeatures) {
    ...
    if (machineConfiguration.isMultiAxisConfiguration() && (useABCPrepositioning || abc.isZero())) {
      ...
      writeBlock(
        ...
        conditional(machineConfiguration.isMachineCoordinate(1), "XXXXX" + abcFormat.format(angles.y)),
        conditional(machineConfiguration.isMachineCoordinate(2), "B" + abcFormat.format(angles.z))
      );
      writeBlock("M71");
...
function onSection() {
  ...
  if (insertToolCall || newWorkOffset || newWorkPlane || forceSmoothing) {
    ...
    if (getProperty("rapidToolChange")) {
      writeRetract(X, Y); // retract
    }
    forceXYZ();
...
function protectedProbeMove(_cycle, x, y, z) {
  ...
  if (_z && z >= getCurrentPosition().z) {
    writeBlock(gFormat.format(65), "P" + 9703, _z, getFeed(cycle.feedrate)); // protected positioning move
  }
  if (_x || _y) {
    writeBlock(gFormat.format(65), "P" + 9703, _x, _y, getFeed(highFeedrate)); // protected positioning move
  }
  if (_z && z < getCurrentPosition().z) {
    writeBlock(gFormat.format(65), "P" + 9703, _z, getFeed(cycle.feedrate)); // protected positioning move
  }
...
function onCyclePoint(x, y, z) {
  ...
  if (isFirstCyclePoint() || isProbeOperation()) {
    ...
    case "probing-x": 
      error(localize("Noch nicht freigegben"));
      protectedProbeMove(cycle, x, y, z - cycle.depth);
      writeBlock(
        gFormat.format(65), "P" + 9700,
    ...
    case "probing-z":
      ...
      writeBlock(
        gFormat.format(65), "P" + 9700,
        "Z" + xyzFormat.format(- cycle.depth - z),
        getProbingArguments(cycle, true)
      );
      break;
    case "probing-x-wall":
    ...
    case "probing-xy-outer-corner":
      ...
      protectedProbeMove(cycle, x, y, z - cycle.depth);
      writeBlock(
        gFormat.format(65), "P" + 9700, xOutput.format(cornerX), yOutput.format(cornerY),
        conditional(cornerI != 0, "I" + xyzFormat.format(cornerI)),
        conditional(cornerJ != 0, "J" + xyzFormat.format(cornerJ)),
        "Q" + xyzFormat.format(cycle.probeOvertravel),
        getProbingArguments(cycle, true)
...
function getProbingArguments(cycle, updateWCS) {
  ...
  return [
    ...
    conditional(outputWCSCode, "W" + probeWCSFormat.format(probeOutputWorkOffset > 6 ? (probeOutputWorkOffset - 6 + 100) : probeOutputWorkOffset + 53))
  ];
}
...
function onCycleEnd() {
  if (isProbeOperation()) {
    zOutput.reset();
    gMotionModal.reset();
    writeBlock(gFormat.format(65), "P" + 9703, zOutput.format(cycle.retract)); // protected retract move
  } else {
    if (cycleSubprogramIsActive) {
      subprogramEnd();
      cycleSubprogramIsActive = false;
  ...
  if (Array.isArray(coolantCodes)) {
    if (singleLineCoolant) {
      ...
      for (var c in coolantCodes) {
        ...
        //writeBlock(gFormat.format(4), " P"(5 * (1 / tool.diameter)));
      }
    }
    return undefined;
  }
...
function writeRetract() {
  ...
  if (method == "G28") {
    ...
    _xHome = machineConfiguration.hasHomePositionX() ? machineConfiguration.getHomePositionX() : toPreciseUnit(266, MM);
    _yHome = machineConfiguration.hasHomePositionY() ? machineConfiguration.getHomePositionY() : toPreciseUnit(-100, MM);
...
function onClose() {
  ...
  writeRetract(Z); // 
  writeBlock(gFormat.format(53), "G00 X156. Y6.");
